...
$stmt = $pdo->prepare("SELECT id, nom, prix, description FROM parfums WHERE id = ?");
$stmt->execute([$id]);
$product = $stmt->fetch();

if (!$product) die("Produit introuvable.");

$nom = trim($_POST['nom'] ?? $product['nom']);
$prix = trim($_POST['prix'] ?? $product['prix']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errors)) {
    $up = $pdo->prepare("UPDATE parfums SET nom = ?, prix = ?, description = ? WHERE id = ?");
    $up->execute([$nom, (float)$prix, $description, $id]);
...
<section class="hero" style="padding:40px 0 10px;">
  <div class="container">
    <h1 style="font-size:44px;">Modifier le produit</h1>
    <p>Édition du parfum #<?= (int)$id ?></p>
  </div>
</section>
